...
Compounding this: the object's dating is itself disputed. It is commonly attributed to the second millennium BCE (Middle-to-Late Minoan period, sometimes cited around 1700 BCE), but because it is a unique, context-poor find with no directly comparable object, this dating should be treated as disputed, not settled. The signs do not closely match Linear A, Linear B, Cretan Hieroglyphic, or any other known script, though a few show superficial resemblance to some Cretan Hieroglyphic signs — any specific resemblance claim needs verification before being relied upon, not treated as settled fact.

There is also a real, published minority scholarly position questioning the disc's authenticity as a genuine ancient artifact — citing the total absence of any comparable find, the unusual stamped-impression manufacturing technique, and some questions raised about Pernier's original excavation documentation. This project treats that question as a live, legitimate, disclosable research thread, not settled in either direction, exactly as the sibling Oak Island project treats its own hoax question. It is SQ-1 below, and it is the literal first task.
