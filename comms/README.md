...
When an analysis pins third-party files (e.g. reference disc images or a comparator sign catalog) by cryptographic checksum, verify the exact repository or raw-download bytes before parsing. Git may rewrite text line endings on checkout — especially with `core.autocrlf=true` — so a checksum failure must be diagnosed before being treated as a source or analysis defect. Do not silently normalize and accept altered bytes: detect known CRLF-only conversion, give a targeted remedy (`core.autocrlf=false` or pinned raw download), and retain the strict expected checksum.
...
**Acting as:** (which agent role's lens this message is written from, e.g. Skeptic, Historian — or "coordinator" for meeting/process messages)
