# Contributing to the Phaistos Disc Collective
...
Rongorongo project opened this process at launch.
...
reviewable work — most usefully, right now, helping with the authenticity
and provenance audit (SQ-1), since that blocks everything else in this
project.
...
1. **Clone or fork** the repository:
   ```
   git clone https://github.com/SoylentAquamarine/phaistos-disc-collective.git
   ```
...
4. **Pick one bounded task** — do not invent a new primary direction for
   ...
   - The authenticity and provenance audit (`config/sidequests.md` SQ-1) —
     compiling the excavation record, dating evidence, and the disputed-
     authenticity minority position, with primary-source citations. This is
     the single most valuable thing a new contributor can do right now, and
     it blocks every other track in this project.
   ...
   - Pick up a named, not-yet-done piece of an open sidequest in
     [`config/sidequests.md`](config/sidequests.md) — most usefully SQ-4,
     the prior-claims catalog, which can proceed in parallel with SQ-1.
   ...
   - **Do not**: edit `knowledge-base/state.md` directly (it only changes
     via PR and review, same rule for everyone), assert or imply a reading
     of the inscription without meeting `methods/falsification-standard.md`,
...
(SQ-1, the authenticity and provenance audit, is the current blocking
priority) or a named follow-up in a recent log or comms entry — do not
touch knowledge-base/state.md directly. Do the work, write it up with the
same disclosure standard as the rest of this repo (checksums, honest null
...
and Rongorongo projects' own `comms/FromClaudeToChatGPT.md` files for what
that looks like in practice over an extended run.

## What this project is not looking for

- Contributions that assert a reading or decipherment without meeting
  [`methods/falsification-standard.md`](methods/falsification-standard.md).
- Treating the disc's authenticity, dating, or any prior claimed reading as
  settled without a primary-source citation — check recent logs for what's
  already been verified before asserting anything as background fact.
...
- A candidate reading built from a handful of resembling signs without
  addressing the fundamental no-second-exemplar problem — the specific,
  well-documented failure mode this project's `agents/skeptic.md` exists to
  catch, and the reason nearly every prior public "decipherment" of this
  object has failed to achieve scholarly acceptance.
