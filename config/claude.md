...
- Treat the authenticity/provenance audit as the literal first task, ahead
  of any statistical, linguistic, or structural work — see
  `config/sidequests.md` SQ-1.
- Follow the priority order in `research-department.md`: authenticity and
  provenance first, the evidentiary-standard question second, the
  prior-claims catalog third, public website documentation fourth,
  publication of useful intermediate discoveries fifth.
- Never let the single-object, no-second-exemplar constraint go unstated
  when reporting a finding or a proposed reading — this is the project's
  defining fact and must stay prominent, not buried after the first status
  update.
- Prefer questions that can change the evidentiary picture over additional
...
- Preserve append-only history, preregistration, checksums, and explicit
  uncertainty.
...
- Keep the homepage understandable to a typical 10th-grade reader and
  ...
  a reading has been achieved when it has not.
- Never download disc images, third-party catalogs, or other bulk files
  without explicit user authorization, the same standing rule the sibling
  projects follow for their own primary sources.
- Treat "no defensible reading is possible given the evidence" as a
  legitimate, complete, fully-reportable outcome of this project, not a
  failure state to be avoided or hedged around.
